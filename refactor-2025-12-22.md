...
1. **Keyboard bindings via `compose_bindings()`** - Not `__init__` mutation
2. **FileLink.Clicked → FileLink.Opened** - Better semantics, unified behavior
3. **ID responsibilities** - FileLink: no auto-gen; CommandLink: auto-gen from name
4. **CommandLink is stateful** - Owns spinner, timers, running state (but not command execution)
5. **set_status(running: bool | None = None)** - Explicit control, no implicit changes
6. **FileLinkList requires IDs** - All items must have explicit IDs (fail fast on duplicates)
7. **add_item(toggled=False)** - Set initial state directly (no wrapper exposure)
8. **Click behavior** - Play/stop controls running; name always opens output
9. **Icon.name preserved** - Not renamed to `id` (avoids confusion)
...
Icon(icon="✅")  # ✗ Error: missing required argument 'name'
[Icon(name="x", ...), Icon(name="x", ...)]  # ✗ Error: Duplicate name 'x'
Icon(name="play", icon="▶️", key="o")  # ✗ Error: key 'o' conflicts with FileLink.open
```
...
FileLinkWithIcons(
    ...
    icons_before=[
        ...
        Icon(name="status", icon="✅", tooltip="Validated", clickable=True, key="1"),
    ],
    icons_after=[
...
class CommandLink(Widget):
    ...
    def __init__(
        ...
        *,
        ...
        status_icon: str = "⚫",            # Initial status icon
        status_tooltip: str | None = None,
        running: bool = False,              # Start in running state?
        show_settings: bool = True,         # Show ⚙️ button
...
- **▶️/⏹️**: Play button (stopped) or Stop button (running) - always shown
- **Name**: Command name (clickable if output_path set, else Static)
- **⚙️**: Optional settings button
...
def set_status(
    ...
    *,
    ...
    >>> cmd.set_status(icon="✅", running=False, tooltip="Passed")  # Stop with status
    >>> cmd.set_status(tooltip="Test 5 of 10...")  # Update tooltip only
    >>> cmd.set_status(icon="❌", tooltip="Failed")  # Update icon (running unchanged)
    """
...
**Widget ID**:
- Auto-generated from name if `id=None`: `CommandLink("Run Tests")` → `id="run-tests"`
- Sanitization: lowercase, spaces→hyphens, keep only alphanumeric/hyphens/underscores
...
# Display: ⚫ ▶️ Run Tests ⚙️

# Start command
cmd.set_status(running=True, tooltip="Running tests...")
# Display: ⠧ ⏹️ Run Tests ⚙️ (spinner animates)

# Complete successfully
cmd.set_status(icon="✅", running=False, tooltip="All tests passed")
cmd.set_output_path(Path("test_results.log"))
# Display: ✅ ▶️ Run Tests ⚙️ (name now clickable)

# Complete with failure
cmd.set_status(icon="❌", running=False, tooltip="3 tests failed")
# Display: ❌ ▶️ Run Tests ⚙️
...
class FileLinkList(VerticalScroll):
    ...
    def __init__(
        ...
        *,
        ...
        show_toggles: bool = False,    # Add ☑/☐ to all items
        show_remove: bool = False,     # Add × to all items
        id: str | None = None,
        classes: str | None = None,    # CSS classes for styling
    ):
```
...
for path in Path(".").glob("*.py"):
    file_list.add_item(
        FileLink(path, id=path.stem),
        toggled=(path.stem == "main")  # Toggle main.py
    )

# ID validation (fail fast)
file_list.add_item(FileLink("file.py"))  # ✗ ValueError: ID required
file_list.add_item(FileLink("dup.py", id="file1"))  # ✗ ValueError: Duplicate ID
...
All widgets support custom keyboard shortcuts via `compose_bindings()` (Textual best practice):
...
class FileLink(Static, can_focus=True):
    ...
    def __init__(self, path, *, open_keys=None, ...):
        # Store keys for compose_bindings()
        self._open_keys = open_keys if open_keys is not None else self.DEFAULT_OPEN_KEYS
        super().__init__(...)
    
    def compose_bindings(self):
        """Generate bindings dynamically (called by Textual)."""
        for key in self._open_keys:
            yield Binding(key, "open_file", "Open file", show=False)

# Instance override
FileLink("file.py", open_keys=["f2"])

# Class override (affects all instances)
FileLink.DEFAULT_OPEN_KEYS = ["f2"]
FileLink("file.py")  # Uses F2
```

**Why `compose_bindings()`?**
- Avoids mutating class-level `BINDINGS` in `__init__`
- Prevents lifecycle timing issues with multiple instances
- Textual best practice for dynamic bindings
...
### ToggleableFileLink → FileLinkList + FileLinkWithIcons
...
### Current CommandLink → New CommandLink
...
cmd.set_status(icon="✅", running=False, tooltip="Passed")
```
...
Main changes:
- `initial_status_icon` → `status_icon`
- `initial_status_tooltip` → `status_tooltip`
...
1. ✅ **FileLink opens files** (exception to display-only rule)
2. ✅ **CommandLink builds its own layout** (doesn't use FileLinkWithIcons)
3. ✅ **CommandLink has NO toggle/remove** (FileLinkList adds them uniformly)
4. ✅ **Icon.name is required** (no auto-naming)
5. ✅ **Fail fast on duplicate names/keys**
6. ✅ **set_status(running: bool | None = None)** (explicit control, no implicit changes)
7. ✅ **Widget ID sanitization** (CommandLink auto-generates from name)
8. ✅ **No icon name sanitization** (keep as provided for clarity)
9. ✅ **Keep FileLinkWithIcons** (useful for files with status/metadata)
10. ✅ **Custom keyboard shortcuts** (via compose_bindings() and DEFAULT_* class attributes)
11. ✅ **FileLinkList is scrollable** (inherits from VerticalScroll)
12. ✅ **Keep set_icon_visible()** (clearer than update_icon for visibility)
13. ✅ **Uniform wrapping** (FileLinkList treats all widget types the same)
14. ✅ **get_icon() raises KeyError** (fail fast, not None)
15. ✅ **add_item(toggled=False)** (set initial toggle state directly)
16. ✅ **FileLinkList requires IDs** (fail fast on missing or duplicate IDs)
17. ✅ **CSS classes parameter** (for styling all widgets)
18. ✅ **FileLink.Opened event** (was Clicked, unified mouse/keyboard behavior)
19. ✅ **compose_bindings()** (not __init__ mutation, Textual best practice)
20. ✅ **CommandLink is stateful** (owns spinner, timers, but not command execution)
21. ✅ **Deterministic click behavior** (play/stop controls running, name opens output)
